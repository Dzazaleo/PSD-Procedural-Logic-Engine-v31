...
import { Handle, Position, useReactFlow, useUpdateNodeInternals, NodeProps } from 'reactflow';
...
export const TargetTemplateNode = memo(({ data, id }: NodeProps<PSDNodeData>) => {
  ...
  // Force handle update when connectivity state changes
  useEffect(() => {
    updateNodeInternals(id);
  }, [id, isConnectable, updateNodeInternals]);
  ...
  const handleFileChange = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 3. Validation: Check for !!TEMPLATE
      if (templateData.containers.length === 0) {
        throw new Error("INVALID TARGET: No top-level group named '!!TEMPLATE' found.");
      }

      // 4. REGISTER WITH STORE FIRST (Critical for 'hasBinary' check)
  ...
  }, [id, setNodes, registerPsd, registerTemplate, updateNodeInternals]);
